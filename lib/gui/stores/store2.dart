...
class Store2 extends StatefulWidget{
  Store2(this.storeStatus);
  String storeStatus;
  @override

    _Store2State createState() => _Store2State(storeStatus);
...
class _Store2State extends State<Store2>{
  ...
  _getDataList(int pageId) {
    GetData.getDataList(funcs.mainLink+'api/getStoreOrders/$storeId/$pageId').then((response) {
      setState(() {
  ...
  _openOrderDetailsPage(String invoiceNumber,
    ...
    Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => orderDetails(invoiceNumber.toString(),Id)));
  }
  ...
  void orderStatusBottomSheet(context, String orderId, String orderNumber){
    ...
    received = Locales.string(context, 'received_title');
    confirmed = Locales.string(context, 'confirmed_title');
    shipped = Locales.string(context, 'shipped_title');
    delivered = Locales.string(context, 'delivered_title');
    canceled = Locales.string(context, 'canceled_title');
          ...
          return Container(
            child: Wrap(
              children: <Widget>[
                ...
                const Divider(),
                ListTile(
                  title: Text(received, style: styles.listTileStyle2, textAlign: TextAlign.center),
                  onTap: (){
                    Navigator.pop(context);
                    _changeOrderStatus(orderNumber,orderId,'1');
                  },
                ),
                const Divider(),
                ListTile(
                  title: Text(confirmed, style: styles.listTileStyle2, textAlign: TextAlign.center),
                  onTap: (){
                    Navigator.pop(context);
                    _changeOrderStatus(orderNumber,orderId,'2');
                  },
                ),
                const Divider(),
                ListTile(
                ...
                const Divider(),
                ListTile(
                    title: Text(canceled, style: styles.listTileStyle2, textAlign: TextAlign.center),
                    onTap: (){
                      Navigator.pop(context);
                      _changeOrderStatus(orderNumber,orderId,'0');
                    }
                ),
                const Divider(),
              ],
            ...
            children: <Widget>[
              Column(
                children: <Widget>[
                  ...
                  //  onTap: ()=> _openOrderDetailsPage(ordersList[index].invoiceNumber.toString(),ordersList[index].orId),
                  ),
...
//                  ):Container(),
  ...
  Widget build(BuildContext context) {
    ...
    IconButton(onPressed: () { Navigator.pop(context); }, icon: Icon(Icons.arrow_back),)
      ...
      drawer: StoreDrawerClass(isLogin, storeName),
    );
  }
