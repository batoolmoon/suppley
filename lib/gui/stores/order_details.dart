...
class orderDetails extends StatefulWidget{
  orderDetails(this.invoiceNumber, this.Id);
  String invoiceNumber;
  String Id;
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _orderDetailsState(invoiceNumber,Id);
  }

}

class _orderDetailsState extends State<orderDetails> {
  _orderDetailsState(this.invoiceNumber,this.Id);
  String Id;
   String invoiceNumber;
  ...
  _getDataList() {
    ...
    GetData.getDataList(funcs.mainLink +
        'api/getOrdersDetails/$theLanguage/$Id/$invoiceNumber').then((
        response) {
      setState(() {
        Iterable list = json.decode(response.body);
        print(json.decode(response.body));
        orderDetailsList =
            list.map((model) => GetOrderProducts.fromJson(model)).toList();
        isLoading = false;
      });
    });
  }
  ...
  getSharedData() async {
    ...
    if (mounted) {
      setState(() {
        //memberId = prefs.getString('storeId')!;
  ...
  getCartTotalPriceAfterOrder() async {
    orderTotalPrice =
    await funcs.getCartTotalPriceAfterOrder(Id, invoiceNumber);
  ...
  Widget widgetProductsData() {
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) =>
          styles.widgetProducts(
              scaffoldKey,
              context,
              isLogin,
              Id,
              double.parse(orderDetailsList[0].currencyExchange),
              orderDetailsList,
              index,
              'orders',
              currencyId),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: styles.theAppBar(
          ...
          theLanguage,
          isLogin,
          context.localeString('order_details') + ' $invoiceNumber',
          true,
          false,
          '0'),
      ...
      bottomNavigationBar: Container(
        ...
        child: Row(
          children: <Widget>[
            Expanded(
              child: ListTile(
                title: Text(context.localeString('total_price'),
                    style: const TextStyle(
                        color: Colors.black87, fontSize: 16.0),
                    textAlign: TextAlign.center),
                subtitle: Text('$orderTotalPrice ' + orderDetailsList[0].symbol,
                    style: const TextStyle(
                        color: Colors.black87, fontSize: 16.0),
                    textAlign: TextAlign.center),
              ),
